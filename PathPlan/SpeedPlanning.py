...
"""
        head_file = '#include "POINTS.h"\n\n'
        point_num = "uint16_t g_AllPoint_num = {};\n".format(len(self.x)+1)
        define = "TYPE_Point_Info_t g_Points[{}]".format(len(self.x)+2)
        others = """={
//          x                y             v             theta            vx             vy
"""
        head = descreb + head_file + point_num + define + others
        points = np.zeros((len(self.x)+1, 6), dtype=np.float)
        points[:-1,0] = self.x
        points[:-1,1] = self.y
        points[:-1,2] = self.v_path_r_cla
        # 因到达最后的一个点前速度不能为0故在最后一个点与倒数第二个点间加
        # 一个点,位置为最后一个点的位置,速度为1/3
        points[-1] = points[-2]
        points[-2,2:] = points[-3,2:] / 3
